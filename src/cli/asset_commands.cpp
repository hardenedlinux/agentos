...
void register_asset_commands(CLI::App& app) {
    ...
    // ---- asset revoke-by-user ----
    // Whole-account hard erasure only (CMS calls this for GDPR
    // right-to-be-forgotten or its own inactivity auto-cleanup) — this is
    // NOT the per-asset "asset revoke" above. It permanently deletes every
    // asset row owned by user_id, including any AgentOS memory-produced
    // assets, not just ones registered via `asset register`. Exposed on
    // the CLI purely so this RPC can be exercised/tested directly; the
    // real caller in production is CMS over the daemon socket, not an
    // operator typing this command.
    {
        auto* revoke_user = asset->add_subcommand(
            "revoke-by-user",
            "Permanently delete ALL assets for a user (whole-account erasure — NOT a per-asset revoke)");
        auto user_id = std::make_shared<std::string>();
        auto confirmed = std::make_shared<bool>(false);
        revoke_user->add_option("user_id", *user_id)->required();
        revoke_user->add_flag("--yes", *confirmed,
            "Skip the interactive confirmation prompt (for scripted/test use)");
        revoke_user->callback([timeout_ms, socket_path, json_flag, access_key, user_id, confirmed] {
            try {
                if (!*confirmed) {
                    std::cerr << agentos::cli::color::yellow(
                        "This permanently deletes ALL assets for user \"" + *user_id
                        + "\" — registered files AND any memory-derived assets. "
                          "This cannot be undone and is not the same as 'asset revoke'.")
                              << "\n";
                    std::cout << "Type the user id again to confirm: ";
                    std::string confirm_input;
                    std::getline(std::cin, confirm_input);
                    if (confirm_input != *user_id) {
                        agentos::cli::die(1, "confirmation did not match — aborted");
                    }
                }
                ...
                auto result = client.send("asset.revoke_by_user", std::move(params));
                if (*json_flag) { print_json(result); }
                else {
                    std::cout << "revoked_count: " << result["revoked_count"].GetInt64()
                              << " (user " << result["user_id"].GetString() << ")\n";
                }
            } catch (const agentos::cli::CliError& e) {
                agentos::cli::die(2, e.what());
            }
        });
        agentos::cli::add_completion(revoke_user);
    }
