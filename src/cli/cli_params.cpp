    ...
    const std::string& acceptance_criteria)
{
    ...
    if (type == "loop") {
        ...
        if (!acceptance_criteria.empty())
            loop.AddMember("acceptance_criteria",
    ...
    int offset)
{
    ...
    if (!type_filter.empty())
        doc.AddMember("type", rapidjson::Value(type_filter.c_str(), a).Move(), a);
...
rapidjson::Document build_worker_toggle_params(const std::string& worker_id)
{
    rapidjson::Document doc(rapidjson::kObjectType);
    auto& a = doc.GetAllocator();
    doc.AddMember("worker_id", rapidjson::Value(worker_id.c_str(), a).Move(), a);
    return doc;
}
    ...
    const std::string& type)
{
    ...
    auto& a = doc.GetAllocator();
    if (!status.empty())
        doc.AddMember("status", rapidjson::Value(status.c_str(), a).Move(), a);
    if (!type.empty())
        doc.AddMember("type", rapidjson::Value(type.c_str(), a).Move(), a);
    return doc;
}
...
rapidjson::Document build_review_id_params(const std::string& review_id)
{
    rapidjson::Document doc(rapidjson::kObjectType);
    auto& a = doc.GetAllocator();
    doc.AddMember("review_id", rapidjson::Value(review_id.c_str(), a).Move(), a);
    return doc;
}
    ...
    const std::string& message)
{
    ...
    auto& a = doc.GetAllocator();
    doc.AddMember("review_id", rapidjson::Value(review_id.c_str(), a).Move(), a);
    doc.AddMember("message",   rapidjson::Value(message.c_str(),   a).Move(), a);
    return doc;
}
...
rapidjson::Document build_forge_status_params(const std::string& forge_id)
{
    rapidjson::Document doc(rapidjson::kObjectType);
    auto& a = doc.GetAllocator();
    doc.AddMember("forge_id", rapidjson::Value(forge_id.c_str(), a).Move(), a);
    return doc;
}
