       ...
       limit, offset, user_filter, show_user, all_jobs, since_minutes]
      {
        try
        {
          ...
          if (!user_filter->empty ())
            ...
            // The daemon scopes job.list to one user_id and rejects a
            // request that names none, unless the caller opts in to a
            // cross-user listing explicitly (tenant isolation). This is an
            // operator tool: no --user means "across all users", so say so.
            params.AddMember ("all_users", rapidjson::Value (true),
                              params.GetAllocator ());
          if (*all_jobs)
