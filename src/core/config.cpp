...
std::optional<Config> load_config(std::string_view path, std::string& error) {
    ...
    try {
        ...
        if (auto* llm_tbl = tbl["llm"].as_table()) {
            ...
            if (auto v = llm_tbl->at_path("rate_limit_max_wait_s").as_integer()) {
                cfg.llm.rate_limit_max_wait_s = static_cast<int>(v->get());
            }
        }
        ...
        // [trusted_workers]
        // Operator-only network sandbox exemption list. Absence of this
        // section, or an absent/empty network_exempt array, leaves
        // cfg.trusted_workers.network_exempt empty — the fail-closed
        // default (no Worker exempt). This section is never written by
        // anything other than a human editing config.toml directly.
        if (auto* tw_tbl = tbl["trusted_workers"].as_table()) {
            if (auto* arr = tw_tbl->at_path("network_exempt").as_array()) {
                for (const auto& elem : *arr) {
                    if (auto v = elem.as_string()) {
                        cfg.trusted_workers.network_exempt.push_back(v->get());
                    }
                }
            }
        }
