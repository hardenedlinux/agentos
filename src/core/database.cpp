...
namespace agentos
{
  ...
  std::optional<Database::AssetDeletionResult>
  Database::delete_all_assets_for_user (const std::string &user_id)
  {
    if (!db_)
      return std::nullopt;
    ...
    auto tx = with_transaction ([&] () -> bool {
      // Distinct hashes this delete touches, captured before the delete
      // so the caller can, post-commit, check whether each is now
      // orphaned across the whole (shared, not per-user) blob store.
      Stmt sel (prepare (
        "SELECT DISTINCT sha256 FROM assets WHERE user_id = ?"));
      if (!sel.s)
        return false;
      sqlite3_bind_text (sel, 1, user_id.c_str (), -1, SQLITE_TRANSIENT);
      while (sqlite3_step (sel) == SQLITE_ROW)
        result.candidate_sha256.push_back (column_text_or_empty (sel, 0));

      // job_assets.asset_id REFERENCES assets(asset_id) with
      // foreign_keys=ON and no ON DELETE clause -- deleting an asset that
      // was ever attached to a job.submit would otherwise fail the FK
      // check on the DELETE below. Clearing job_assets here is safe: the
      // per-job materialized copy lives as its own file under
      // jobs/<job_id>/assets/, independent of this index row.
      Stmt del_ja (prepare (
        "DELETE FROM job_assets WHERE asset_id IN "
        "(SELECT asset_id FROM assets WHERE user_id = ?)"));
      if (!del_ja.s)
        return false;
      sqlite3_bind_text (del_ja, 1, user_id.c_str (), -1, SQLITE_TRANSIENT);
      if (sqlite3_step (del_ja) != SQLITE_DONE)
      {
        spdlog::error ("[database] delete_all_assets_for_user: job_assets "
                       "cleanup failed: {}",
                       sqlite3_errmsg (db_));
        return false;
      }

      Stmt del_a (prepare ("DELETE FROM assets WHERE user_id = ?"));
      if (!del_a.s)
        return false;
      sqlite3_bind_text (del_a, 1, user_id.c_str (), -1, SQLITE_TRANSIENT);
      if (sqlite3_step (del_a) != SQLITE_DONE)
      {
        spdlog::error ("[database] delete_all_assets_for_user: assets "
                       "delete failed: {}",
                       sqlite3_errmsg (db_));
        return false;
      }
      result.deleted_count = sqlite3_changes (db_);
      return true;
    });
  ...
  int64_t
  Database::count_assets_by_sha256 (const std::string &sha256)
  {
    if (!db_)
      return -1;
    Stmt stmt (prepare ("SELECT COUNT(*) FROM assets WHERE sha256 = ?"));
    if (!stmt.s)
      return -1;
