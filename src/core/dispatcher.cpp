...
namespace agentos
{
  ...
  DispatchResult Dispatcher::fork_exec (const DispatchRequest &req)
  {
    if (req.job_id.empty ())
    {
      const std::string err = "job_id is required";
      spdlog::error ("[dispatcher] {}", err);
      return {false, -1, {}, {}, {}, err};
    }

    const fs::path job_path = agentos_home () / "jobs" / req.job_id;
    const fs::path run_path = run_dir (req.run_id);
    ...
    // Ensure the per-job workspace exists.
    {
      std::error_code ec;
      fs::create_directories (job_path, ec);
      if (ec)
      {
        const std::string err = "cannot create job dir: " + ec.message ();
        spdlog::error ("[dispatcher] {}", err);
        return {false, -1, {}, {}, {}, err};
      }
    }
    ...
    {
      const std::string err = std::string ("pipe2 failed: ") + strerror (errno);
      spdlog::error ("[dispatcher] {}", err);
      close (log_fd);
      return {false, -1, {}, {}, {}, err};
    }
    ...
    if (pid == 0)
    {
      ...
      // array — no filesystem access, safe before the sandbox is applied.
      setenv ("AGENTOS_HOME", agentos_home ().c_str (), 1);
      setenv ("AGENTOS_RUN_DIR", run_path.c_str (), 1);
    ...
    // Write task_json; child reads from stdin until EOF.
    {
      ...
      if (!write_full (pipe_fds[1], buf, left))
        spdlog::warn ("[dispatcher] write to child stdin failed: {}",
                      strerror (errno));
      close (pipe_fds[1]); // EOF — child's json.load(sys.stdin) can return
    }

    spdlog::info (
      "[dispatcher] forked job_id={} run_id={} pid={} step_id={} binary={}",
      req.job_id, req.run_id, pid, req.step_id, req.binary_path);

    // Record in-flight entry — protected by mutex (reap() runs on another
    // thread).
    {
      std::lock_guard<std::mutex> lk (mutex_);
    ...
    return {
      true, pid, job_path.string (), run_path.string (), log_path.string (),
      {}};
  }
  ...
  CollectResult Dispatcher::collect (const std::string &run_id,
                                     ...
                                     int exit_code)
  {
    // ADR-016 Result File Wire Format — collect() rules:
    //   exit_code != 0                     -> failed; result.json not read
    //   exit_code == 0, result.json absent -> failed (Worker Contract
    ...
    {
      const std::string err
        = "worker exited with code " + std::to_string (exit_code);
      spdlog::warn ("[dispatcher] run_id={} {}", run_id, err);
      return {false, exit_code, {}, err};
    }

    const fs::path result_path = fs::path (run_dir_path) / "result.json";
    ...
    {
      const std::string err = "result.json is not a valid {status,result} "
                              "envelope: "
                              + result_path.string ();
      spdlog::error ("[dispatcher] run_id={} {}", run_id, err);
      return {false, exit_code, {}, err};
    ...
    if (status == "error")
    {
      std::string err = "worker reported status=error";
    ...
    // status == "ok": serialise the `result` field alone — output_schema
    // (ADR-019) describes result.json.result, not the envelope.
    ...
    spdlog::info ("[dispatcher] collected run_id={} exit_code={} status=ok",
                  run_id, exit_code);
    return {true, exit_code, buf.GetString (), {}};
  }
