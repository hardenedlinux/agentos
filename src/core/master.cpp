      ...
      const std::vector<RegisteredAdviser> &candidates) const
  {
    ...
    req.system_prompt
      = "You are Master's Digest Pass for an agent orchestration system. "
        ...
        "2) classify \"deliverable_kind\" as exactly \"artifact\" or "
        "\"result\" — there are only ever these two kinds of task. "
        "\"artifact\" means the user wants the SOURCE CODE itself as the "
        "deliverable (e.g. \"write me a function/script/program that "
        "does X\") — nothing else ever qualifies as \"artifact\". "
        "\"result\" means everything else: the user wants the actual "
        "output of doing something with real input — translated text, "
        "a written document, an analysis, a generated image or video, "
        "a computed answer, any binary file — even when that output is "
        "itself referenced by a file path inside the JSON result rather "
        "than inlined as text. A generated image, document, or video is "
        "\"result\", never \"artifact\" — \"artifact\" is reserved "
        "exclusively for source code; "
