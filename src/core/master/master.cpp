...
// RapidJSON for building/parsing planning context and plan JSON
...
TaskResult Master::submit(const Task& task) {
    ...
    if (!adviser) {
        spdlog::error("[master] no adviser registered for domain 'general'");
        return { task.id, false, "", "no adviser available" };
    }
    spdlog::info("[master] routing to adviser: {} ({})", adviser->name, adviser->id);
    ...
    if (!verify.ok) {
        ...
        spdlog::error("[master] plan verification failed:{}", errs);
...
std::optional<Plan> Master::request_plan(const RegisteredAdviser& adviser,
                                          const Task&            task) {
    ...
    dispatcher_.send_request(
        ...
        [&](const std::string& result, const std::string& error) {
            if (!error.empty()) {
                spdlog::error("[master] adviser {} returned error: {}", adviser.id, error);
            } else {
                plan_json = result;
            }
            got_response = true;
...
std::string Master::build_command_context() const {
...
std::optional<Plan> Master::parse_plan(const TaskId&      task_id,
    ...
    if (doc.Parse(plan_json.c_str()).HasParseError()) {
        spdlog::error("[master] failed to parse plan JSON");
        return std::nullopt;
    }
    ...
    if (!doc.HasMember("steps") || !doc["steps"].IsArray()) {
        spdlog::error("[master] plan JSON missing 'steps' array");
        return std::nullopt;
    }
