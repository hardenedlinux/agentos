...
#include "agentos/syscall_whitelist.h"
  ...
  // Resolve a filesystem path declared by a Suite manifest.
  // Manifest paths are AgentOS-relative unless they are absolute.
  static std::string
  resolve_manifest_path (const std::string &path)
  {
    ...
    return (agentos_home () / path).lexically_normal ().string ();
  }
                  ...
                  const std::vector<int> &allowed_tcp_ports)
  {
    ...
    // Add allowed read paths. Landlock PATH_BENEATH rules must use
    // access rights appropriate for the referenced inode: READ_DIR for
    // directories and READ_FILE for regular files.
    for (const auto &path : allowed_read_paths)
    {
      ...
      struct stat st {};
      if (fstat (path_attr.parent_fd, &st) < 0)
      {
        spdlog::warn ("[sandbox] fstat {} for landlock failed: {}", path,
                      strerror (errno));
      ...
      {
        spdlog::warn (
          "[sandbox] unsupported fs_read target {} (not a regular file or "
          "directory)",
          path);
        close (path_attr.parent_fd);
        continue;
    ...
    // Add allowed write paths. Directories need directory operations;
    // regular files only need file read/write operations.
    for (const auto &path : allowed_write_paths)
    {
      ...
      {
        spdlog::warn (
          "[sandbox] unsupported fs_write target {} (not a regular file or "
          "directory)",
          path);
        close (path_attr.parent_fd);
        continue;
                             ...
                             bool network, const std::string &run_id)
  {
    ...
    read_paths.reserve (fs_read.size () + 8);
    write_paths.reserve (fs_write.size () + 1);

    for (const auto &path : fs_read)
      read_paths.push_back (resolve_manifest_path (path));

    for (const auto &path : fs_write)
      write_paths.push_back (resolve_manifest_path (path));

    // Every worker owns its job workspace and run workspace. These are
    // implicit runtime capabilities; manifest fs_write is reserved for
    // additional resources outside the current job/run workspaces.
