...
namespace agentos
{
  ...
  public:
    ...
    // -- Whole-account asset erasure -------------------------------------------
    // Two triggers only: a GDPR right-to-be-forgotten request, or the
    // inactivity auto-cleanup job (window TBD: 3/6/12 months). Both mean
    // the entire account is going away, so this is a HARD delete — unlike
    // asset.revoke's soft-delete (status='revoked'), which exists so a
    // job that already materialized that asset keeps working. There is no
    // "future job" once the account is gone, so nothing needs to keep
    // working.
    struct AssetDeletionResult
    {
      int64_t deleted_count = 0;
      // Distinct sha256 hashes that were touched by the delete. These are
      // candidates only, not confirmed-orphaned: the same content-
      // addressed blob may still be referenced by another asset row —
      // this user's or another user's — so each must be re-checked via
      // count_assets_by_sha256 AFTER this call returns before its blob
      // directory can be safely removed from disk.
      std::vector<std::string> candidate_sha256;
    };

    // Deletes every assets-table row owned by user_id, and the job_assets
    // rows that reference those asset_ids (required: job_assets.asset_id
    // REFERENCES assets(asset_id) with foreign_keys=ON and no ON DELETE
    // clause, so deleting an asset ever attached to a job.submit would
    // otherwise fail the FK check). Both deletes run in one transaction.
    // A per-job materialized copy under jobs/<job_id>/assets/ is a
    // physically separate file, independent of the job_assets index row,
    // so clearing job_assets here never touches anything an
    // already-completed job reads.
    //
    // Returns std::nullopt only on an actual DB failure (prepare/step/
    // commit error). A user_id with zero assets is a normal success with
    // deleted_count == 0 and an empty candidate_sha256 — not a failure.
    std::optional<AssetDeletionResult>
    delete_all_assets_for_user (const std::string &user_id);

    // Number of asset rows, across ALL users, that still reference
    // sha256. The blob store is shared/content-addressed, not
    // per-user, so this is the only correct way to decide whether a blob
    // directory is safe to remove from disk after a delete.
    // Returns -1 on a DB failure — callers must treat that as "unknown,
    // leave the blob in place," never as "0 references."
    int64_t count_assets_by_sha256 (const std::string &sha256);
