...
namespace agentos::forge
{
  ...
  public:
    ...
    ForgeCoordinator (Database &db, LlmProxy &llm, Registry &registry,
    ...
    // Signal the background thread to stop and join. Safe to call from any
    // thread.
    void stop ();
  ...
  private:
    ...
    // Populates job.writer_output_json, job.reviewer_verdict_json,
    // job.feedback.
    ...
    bool call_code_writer (ForgePipelineJob &job, int &tokens_prompt,
    ...
    // Populates tokens_prompt/tokens_completion with LLM usage.
    bool call_code_reviewer (ForgePipelineJob &job, int &tokens_prompt,
                             int &tokens_completion);

    // Enforce Layer pre-check (ADR-009 Layer B):
    // Validate capability declaration against ADR-006 policy before Reviewer
    // runs. Returns false (and sets job.feedback) if network:true or exec:true
    // declared.
    bool enforce_capability_policy (ForgePipelineJob &job);

    /// Python syntax pre-check: runs ast.parse on impl file before Reviewer.
    /// Catches SyntaxError early to avoid wasting a Reviewer LLM call.
    /// Returns true if syntax valid (or non-.py file); sets job.feedback on
    /// failure.
    bool syntax_precheck (ForgePipelineJob &job);
