...
namespace agentos
{
  ...
  public:
    ...
    /// @param rate_limit_max_wait_s  ADR-017: cap on total time spent retrying
    ///        a DeepSeek 429 (concurrency-limit) response; irrelevant to
    ///        every other provider. Defaulted so existing call sites that
    ///        haven't been updated yet still compile.
    explicit LlmProxy (int pool_size, int timeout_s,
                       int rate_limit_max_wait_s = 600)
      : timeout_s_ (timeout_s), rate_limit_max_wait_s_ (rate_limit_max_wait_s)
    {
