...
private:
  ...
  // Looks up the Worker for the step's command, calls dispatcher_.fork_exec(),
  // records worker_runs in DB.
  void dispatch_next_step (ActiveJob &job);

  // Called on WorkerDone: read result, store in DB, advance pipeline.
  void on_step_complete (const std::string &job_id,
                         const std::string &run_id,
                         int                exit_code,
                         const std::string &job_dir);

  // Called on WorkerFailed: try next Worker or escalate to Master.
  void on_step_failed (const std::string &job_id,
                       const std::string &run_id,
                       int                exit_code);
  ...
  void reply_ok    (const std::string &identity,
                    const std::string &request_id,
