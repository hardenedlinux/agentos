...
struct Loop {
    ...
    std::optional<std::string> last_feedback; // Reviewer feedback from last rejection
};
...
struct Job {
    ...
    std::string phase;         // planning | executing | repairing | done | failed | human_review
...
struct ForgeJob {
    ...
    std::string phase;               // drafting | reviewing | promoted | rejected | human_review
    int         attempt = 0;
    int         max_attempts = 0;
