...
namespace agentos
{
  ...
  // fmt v10+ format_as support — allows spdlog::info("{}", some_strong_id)
  template <typename Tag> const std::string &format_as (const StrongId<Tag> &id)
  {
    return id.value ();
  }
...
namespace std
{
  template <typename Tag> struct hash<agentos::StrongId<Tag>>
  {
