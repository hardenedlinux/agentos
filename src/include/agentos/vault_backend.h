...
public:
    ...
    /// On every startup: load persistent state, unseal and return plaintext vault_key.
...
std::unique_ptr<VaultBackend> create_vault_backend(const Config::Vault &cfg,
...
// ---------------------------------------------------------------------------
// HardwareTPMBackend / HardwareTPMPCRBackend (compiled only with hardware TPM)
// ---------------------------------------------------------------------------
