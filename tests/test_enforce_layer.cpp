...
namespace agentos
{
  namespace
  {
    ...
    protected:
      ...
      // db must be declared before registry — members init in declaration order
      MockDatabase db;
      Registry registry{db}; // no MockRegistry needed; Registry is the real one
    ...
    {
      EXPECT_TRUE (
        enforceLayer.validate_sandbox_probe ("job1", "int main() {}"));
    }

    TEST_F (EnforceLayerTest,
            HumanEscalationRequired_AttemptExceedsMax_ReturnsTrue)
    {
      EXPECT_TRUE (enforceLayer.human_escalation_required ("job1", 4, 3));
    }

    TEST_F (EnforceLayerTest,
            HumanEscalationRequired_AttemptWithinMax_ReturnsFalse)
    {
