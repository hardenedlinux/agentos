 ...
 *   - Enforce Layer policy rejection (network:true, exec:true, contradictory ports)
...
{
  // human_reviews.forge_id references forge_pipeline_jobs(id) — insert parent first.
...
protected:
  ...
  void SetUp () override
  {
    ...
    seed ("code-writer",
          ...
          "Schema: {understanding, language, entry_point, code, capability, notes}\n"
          "language must be python or guile. capability: {network,exec,fs_read,fs_write}\n");
    seed ("code-reviewer",
